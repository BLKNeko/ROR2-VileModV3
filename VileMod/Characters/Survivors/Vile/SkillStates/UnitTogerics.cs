...
{
    public class UnitTogerics : BaseSkillState
    {
        ...
        public override void OnEnter()
        {
            ...
            if (VBC.GetBoltValue() < boltCost)
            {
                ...
                Chat.AddMessage($"You need at least {boltCost} Vile Bolts to call Togerics unit! You currently have {VBC.GetBoltValue()} Vile Bolts.");
        ...
        private void Fire()
        {
            if (!hasFired)
            {
                ...
                if (isAuthority)
                {
                    ...
                    ShotgunProjectille.projectilePrefab = VileAssets.unitTogericsPrefab;
                    ShotgunProjectille.position = spawnPos;
                    ShotgunProjectille.rotation = Util.QuaternionSafeLookRotation(Vector3.zero);
