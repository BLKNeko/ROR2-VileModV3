...
namespace VileMod.Survivors.Vile.SkillStates
{
    ...
    {
        public static float damageCoefficient = VileStaticValues.VNapalmBombDamageCoefficient;
        ...
        private void Fire()
        {
            if (!hasFired)
            {
                ...
                if (isAuthority)
                {
                    ...
                    PlayCrossfade("Gesture, Override", "VileGranade", playbackRateParam, duration * 0.9f, 0.2f * duration);

                    Ray aimRay = GetAimRay();

                    FireProjectileInfo VProjectille = new FireProjectileInfo();
                    VProjectille.projectilePrefab = VileAssets.NapalmBombProjectile;
