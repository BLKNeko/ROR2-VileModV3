    ...
    public class FrontRunner : BaseSkillState
    {
        ...
        private void FireES()
        {
            if (!this.hasFired)
            {
                ...
                if (base.isAuthority)
                {
                    ProjectileManager.instance.FireProjectile(Modules.Projectiles.FrontRunnerFireBallProjectile, aimRay.origin, Util.QuaternionSafeLookRotation(aimRay.direction), base.gameObject, this.damageCoefficient * this.damageStat, 0f, Util.CheckRoll(this.critStat, base.characterBody.master), DamageColorIndex.Default, null, -1f);
                }
            }
        }
